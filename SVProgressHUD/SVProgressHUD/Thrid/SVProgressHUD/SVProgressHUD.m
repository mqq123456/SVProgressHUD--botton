...
+ (void)showBottonStatus:(NSString *)status {
    [self sharedView];
    [self showProgress:SVProgressHUDUndefinedProgress status:status];
}
...
+ (void)showWithBottonStatus:(NSString*)status {
    NSTimeInterval displayInterval = [self displayDurationForString:status];
    [[self sharedView] showImage:nil status:status duration:displayInterval];
}
+ (void)showWithStatus:(NSString*)status type:(SVProgressHUDCenterType)type{
    NSTimeInterval displayInterval = [self displayDurationForString:status];
    [[self sharedView] showImage:nil status:status duration:displayInterval centerTyle:type];
}
...
- (void)moveToPoint:(CGPoint)newCenter rotateAngle:(CGFloat)angle {
    ...
    if (self.centerStyle == SVProgressHUDCenterBotton) {
        self.hudView.center = CGPointMake(self.bounds.size.width/2, self.bounds.size.height -100);
    }else{
...
- (void)showImage:(UIImage*)image status:(NSString*)status duration:(NSTimeInterval)duration centerTyle:(SVProgressHUDCenterType)centerType {
    self.centerStyle = centerType;
    [self showImage:nil status:status duration:duration];
}
- (void)showImage:(UIImage*)image status:(NSString*)status duration:(NSTimeInterval)duration {
    __weak SVProgressHUD *weakSelf = self;
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
...
- (void)setCenterStyle:(SVProgressHUDCenterType)centerStyle {
    if (!_isInitializing) _centerStyle = centerStyle;
}
- (void)setDefaultStyle:(SVProgressHUDStyle)style {
